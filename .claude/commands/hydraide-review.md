...
- **Race in shift patterns**: read-many followed by `CatalogShiftBatch` without a lock is a data-loss race. Either use `CatalogShiftExpired` (atomic), or wrap in `Lock`/`Unlock`.
