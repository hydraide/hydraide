...
- **No persistence.** The bucket is derived state, kept only while the swamp is summoned. Closing the swamp drops every bucket; the next filter rebuilds.

### What it does, in one paragraph

When you query `asn = 42` for the first time, the swamp takes a snapshot of its in-memory key index, body-decodes every Treasure once, and stores `(canonical-value → set-of-Treasure-keys)` in a Go map. That first call pays the full body-pass cost — the same cost a non-bucket filter would have paid. Every subsequent call for any value on the same field is then a map lookup plus a stable-sort of the small result set. Mutations (`Save`, `Patch`, delete) refresh every initialised bucket through the same `SaveFunction` hook the existing beacons use, so the index is always consistent with the data without a rebuild.
...
| AND of one indexable leg plus other legs | Bucket lookup on the indexable leg, residual = the rest |
| OR where every leg is `Equal` / `IN` on a body field | Bucket lookup per leg, deduplicated union |
...
| Only range, `NOT_EQUAL`, `CONTAINS`, vector, geo, phrase, nested-slice | Bypass — falls back to the legacy beacon walk |
...
The bucket index trades memory for lookup speed. Before reaching for it, ask whether the workload would be better served by **sharding** — splitting the data across multiple swamps so the filter axis becomes the swamp name.
...
| Hot multi-tenant data | **Sharding by tenant** plus auto-bucket inside each tenant swamp | Isolation + cache locality first, then index acceleration on the secondary fields |
...
- **Field values must be one of the supported canonical kinds.** `bool`, `int8..int64`, `uint8..uint64`, `float32/64`, `string`, `nil`. Anything else — slices, maps, time.Time embedded in a struct — collapses to the null bucket and queries on real values return no rows.
- **The field must actually exist in the body.** If half the rows omit the field, those rows go into the "null" slot. A filter like `asn = 42` matches the value slot, not the null slot — which is what you want — but a filter `asn IS_EMPTY` is not bucket-indexed at all and stays on the bypass route.
...
- `Equal` without `BytesFieldPath`. The proto allows comparing against Treasure-level scalar fields (the typed value field of a Treasure). Those are not body fields; the planner skips the bucket route.
- `Equal` against a `CreatedAt` / `UpdatedAt` / `ExpiredAt` timestamp value. Same reason — it's metadata, not body.
- `EQUAL` on a numeric value where the stored encoding does not match the Go type the client sent (e.g. msgpack persisted the value as a `string`, the filter compares against an `int`). The canonical normalisation collapses numeric kinds across signedness and lossless float conversion, but it never crosses the string/number boundary.
...
- **Mutation cost.** Every `Save` decodes the body once per initialised bucket on that swamp, to refresh the index. One bucket per swamp adds ~1 µs to a Save. Five buckets — five body decodes per Save. Plan accordingly when registering many fields' worth of buckets.
...
A bucket is safe to query while mutations land. The build runs on a snapshot of the swamp's primary key index; mutations that arrive during the build are buffered and replayed in FIFO order at the end. Two callers that race on the same first lookup cooperate via a `sync.Once` inside the bucket — only one snapshot, one build. Two callers building different field paths run in parallel without blocking each other.

The internal lock order is `bucketsMu → bucket.mu → bucket.pendingMu`. A lookup acquires `bucket.mu.RLock`, copies the matching slice, releases the lock, and works on the copy — so a `LookupEqual` never blocks a concurrent `Save`.

### Verification

Live results on a dev compose instance are in [`docs/benchmarks/V2_BUCKET_RESULTS.md`](../benchmarks/V2_BUCKET_RESULTS.md). 30 of 30 smoke + bench checks pass, including matrix correctness, mutation propagation, multi-bucket sync, sequential builds, lifecycle (re-summon rebuild), and concurrent cold builds.
