...
func TestV1ToV2Migration_SmallSwamp(t *testing.T) {
	...
	if err := v2Chron.Close(); err != nil {
		t.Fatalf("failed to close V2 chronicler: %v", err)
	}
...
func TestV1ToV2Migration_Persistence(t *testing.T) {
	...
	// Close to flush buffer (simulates graceful shutdown)
	if err := v2Chron1.Close(); err != nil {
		t.Fatalf("failed to close V2 chronicler: %v", err)
	}
